...
import me.navid.scrambilo.ui.hints.HintDialogFragment
...
class GameFragment : DaggerFragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        with(viewModel) {
            if (!userWords.isNullOrEmpty()) wordAdapter.addAll(userWords.toList())
    ...
    private fun onAnswer(result: GameViewModel.ResultType) {
        ...
        when (result) {
            GameViewModel.ResultType.Correct -> {
                wordAdapter.add(wordAdapter.itemCount, result.data!!)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.action_menu_hint -> {
                HintDialogFragment.newInstance(viewModel.getAllWords()).show(childFragmentManager, null)
                true
            }
