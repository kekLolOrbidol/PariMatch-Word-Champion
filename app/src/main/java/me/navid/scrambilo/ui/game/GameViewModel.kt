...
import me.navid.scrambilo.repository.DictionarRepo
import java.util.*
import javax.inject.Inject

class GameViewModel @Inject constructor(
    boardRepository: BoardRepository,
    dictionarRepo: DictionarRepo
) : ViewModel() {
    companion object {
        private const val SIZE = 5
    }

    private val map: Graphic
    private val language = Language.English
    private val trie: LiveData<Result<Trie>>
    private val difficulty = Difficult.Medium
    private val solveBoardResult: LiveData<Result<List<Term>>>

    val board: Array<Alphabets>
    ...
    init {
        this.map = Graphic(SIZE, SIZE)
        this.board = boardRepository.createBoard(language, SIZE, difficulty.range)
        this.trie = dictionarRepo.createDictionary(language)
