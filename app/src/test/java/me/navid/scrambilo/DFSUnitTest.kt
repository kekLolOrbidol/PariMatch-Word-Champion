...
import me.navid.scrambilo.model.Graphic
...
class DFSUnitTest {
    ...
    private lateinit var map: Graphic
    private lateinit var trie: Trie
    private lateinit var alphabets: Array<Alphabets>
    ...
    private fun createGraph(): Graphic {
        val g = Graphic(SIZE, SIZE)
        g.addEdge(0, 1)
        g.addEdge(0, 5)
        g.addEdge(1, 0)
        g.addEdge(1, 2)
    ...
    private fun createTable(lang: Language): Array<Alphabets> {
        return when (lang) {
            Language.ENGLISH -> Array(SIZE) { lang.alphabets[Random.nextInt(lang.alphabets.size)] }
            Language.PERSIAN -> {
    ...
    enum class Language(val alphabets: Array<Alphabets>, val file: String) {
        ENGLISH(ENGLISH_ALPHABET.map { Alphabets(it, 1, 1) }.toTypedArray(), "corncob_lowercase.txt"),
